...
- [ ] 1.1 Create `src/domain/`. Merge `schema/group-size.ts` + `schema/identifiers.ts`
      → `domain/primitives.schema.ts` (`GroupSize`, `CourseId`, `PlatformId`).
- [ ] 1.2 Move `schema/scraped-tee-time.ts` → `domain/tee-time.schema.ts`
      (`BaseTeeTime`, `ScrapedTeeTime`); move `schema/scraped-tee-time.test.ts` →
      `domain/tee-time.schema.test.ts`.
- [ ] 1.3 Move `platforms/course-booking-config.ts` → `domain/course-config.base.ts`
      and rename the type `CourseBookingConfig` → `CourseConfig`.
- [ ] 1.4 Move `platforms/booking-platform-scraper.ts` →
      `domain/booking-platform-scraper.port.ts`.
- [ ] 1.5 Move `platforms/best-booking-url.ts` → `domain/best-booking-url.util.ts`.
- [ ] 1.6 Move `time/build-local-start-instant.ts` →
      `domain/local-start-instant.util.ts`.

## 2. Transport layer

- [ ] 2.1 Create `src/transport/`. Move `platforms/json-fetcher.ts` →
      `transport/json-fetcher.port.ts`.
- [ ] 2.2 Move `platforms/playwright-json-fetcher.ts` →
      `transport/playwright-json-fetcher.adapter.ts`.

## 3. Chronogolf V1 platform

- [ ] 3.1 Rename `platforms/chronogolf-v1/chronogolf-v1-config.ts` →
...
- [ ] 3.2 Fold `build-tee-times-request-url.ts`, `parse-response.ts`
...
- [ ] 4.3 Delete `parse-response.test.ts` and `merge-listing.test.ts`.

## 5. Wire-up and cleanup

- [ ] 5.1 Update all import specifiers across the package to the new paths,
      including `.js` extension specifiers.
- [ ] 5.2 Remove the now-empty `src/schema/`, `src/time/` directories (and the
      framework files that left `src/platforms/`).
- [ ] 5.3 Confirm no `package.json` `exports` map exists to update (out of scope —
      none exists and nothing consumes this package yet). No edit expected.

## 6. Verify (no behavioural diff)

- [ ] 6.1 Run `pnpm format`, `pnpm check-types`, `pnpm lint`, `pnpm test`; fix any
