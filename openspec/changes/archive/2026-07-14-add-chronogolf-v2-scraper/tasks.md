...
- [x] 2.1 Add `platforms/chronogolf-v2/chronogolf-v2-course-config.ts` (`ChronogolfV2CourseConfig extends CourseConfig` with `courseIds: string[]`, `slug: string`, `affiliationTypeId: number`, scrape `tld: string`, and user-facing `bookingTld: string`)
...
- [x] 2.7 Implement per-group-size `bookingUrls`: build a reservation-review deep link `https://www.chronogolf.<bookingTld>/club/<slug>/booking/?source=chronogolf&medium=profile#/teetime/review?affiliation_type_ids=<id×players>&teetime_id=<id>&nb_holes=<holes>&engine=2` (host from the canonical `bookingTld`, never the scrape `tld` mirror), selected via `bestBookingUrl(deepLink, config.bookingPortalUrl)`
...
- [x] 4.1 In `apps/scraper-local/src/main.ts`, construct `ChronogolfV2Scraper` with The Willows config and the existing host-limited browser `JsonFetcher` (`limitedFetcher`), and add it to the orchestrator's scraper array
- [x] 4.2 Add The Willows' pricing config to the `PricingEngine` course-config map

## 5. Verify

- [x] 5.1 Run `pnpm format`, `pnpm check-types`, and `pnpm lint`; fix all errors
- [x] 5.2 Run the full test suite green
- [x] 5.3 Run `apps/scraper-local` against local DynamoDB and confirm The Willows records land for a future date (9- and 18-hole records across loops, `onlineBookable: true`, 9-hole `pricePerPlayer` populated after tax, 18-hole `pricePerPlayer: null`)
