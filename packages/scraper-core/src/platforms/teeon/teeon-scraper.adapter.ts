 ...
 * injected {@link CapturedJsonFetcher} and consumes the SPA's own captured
 * `guest/tee-time` response. TeeOn serves no per-slot price, so every record
 * carries `dynamicPrice: null`.
 */
export class TeeOnScraper implements BookingPlatformScraper {
  ...
  async scrape(courseId: CourseId, date: string): Promise<ScrapedTeeTime[]> {
    ...
    const responseUrlPrefix = `${GUEST_TEE_TIME_ENDPOINT}?facility_id=${config.facilityId}&date=${date}`;

    const json = await this.fetcher.capture(portalUrl, responseUrlPrefix);
    const rows = parseResponse(json);

    const scrapedAt = new Date().toISOString();
    return rows.flatMap((row) => buildRecords(row, config, portalUrl, scrapedAt));
  }
}
...
const RawTeeOnTeeTime = z.object({
  ...
  date: z.string(),
  // Remaining bookable slots at this start; caps the contiguous group sizes.
 ...
 * alone. Both records share the start's contiguous group sizes and the single
 * portal-with-date booking URL, and carry `dynamicPrice: null`.
 *
 ...
 * @returns The 9-hole record, plus the 18-hole record when a continuation exists.
 */
function buildRecords(
  ...
  const groupSizes = buildGroupSizes(row.quantity_remaining);
...
/** The contiguous range `[1 .. min(quantityRemaining, 4)]` of bookable group sizes. */
function buildGroupSizes(quantityRemaining: number): GroupSize[] {
  const count = Math.min(quantityRemaining, MAX_GROUP_SIZE);
